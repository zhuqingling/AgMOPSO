...
public class Spea2_SDEFitness {
   ...
   */
  public Spea2_SDEFitness(SolutionSet solutionSet) {
    distance     = distance_.SDE_distanceMatrix(solutionSet);        
  ...
  public void fitnessAssign() {
    ...
    for (int i = 1; i < distance.length; i++) {
        ...
        if(distance[i][0] < min_distance)
        	min_distance = distance[i][0];
    }*/
    for (int i = 0; i < distance.length; i++) {
      Arrays.sort(distance[i]);
      //kDistance = 1.0 / (distance[i][k] + Math.abs(min_distance) + 2.0); // Calcule de D(i) distance
  ...
  */
  public SolutionSet environmentalSelection(int size){        
    ...
    double [][] distance = distance_.SDE_distanceMatrix(aux);   
